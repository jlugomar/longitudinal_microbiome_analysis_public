function [BayesNet, outstats] = ExhaustiveFullNetSearchGlobal(contData, discData, ...
    priorPrecision, phencol, searchParameter)
%[BayesNet, outstats] = ExhaustiveFullNetSearchGlobal(contData, discData, priorPrecision, phencol, searchParameter)
%
% Performs a greedy hillclimbing search over all possible edges, adding/removing 
% one edge at a time if it improves BIC score.  In contrast to the usual learning 
% algorithm that uses a K2-style node order, this one builds the most likely
...
%
% Modified by Jose Lugo-Martinez under the MIT license. 
%
...
while (~done && (currentPenalizedScore > maxScore || invalidOperation))
    ...
    % Greedy choice for candidate edge to add/remove 
    newEdgeDiffScore = max(max(search.lldiff));
    if (newEdgeDiffScore > bf_thresh)
