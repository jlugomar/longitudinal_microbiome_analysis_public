function DBN = bootstrapLearnHeterogeneousDynamicBayesNetworkParallel(filename, pheno, nboots, edgeThreshold, cols2learn, maxParents, localNetStruc, intraEdges, mle, nu, alpha, sigma2, BFTHRESH, MAXDISCVALS, verbose)
...
% G = bootstrapLearnDynamicBayesNetwork('infant_gut_microbiota_dbn_sample_alignment_filtered_sr5d', '', 10, 0.7, 5, false, false, false, 1, 1, 1); %MAP
  ...
  fprintf(1, 'Starting Boot Sample No. %d\n',b);
        ...
        currFilename = [filename, '_boot', num2str(i)];
       
        
        %% Take sample data and split into two (overlapping) time points
        [d0, dn, ~] = MakeTSBNData(sampleData, sampleSubjectIDs);
        searchParameter = struct('d0',d0,'dn',dn,'DBN',true,'nophenotype',true,'annealing',false,'local',localNetStruc,'bic',mle,...
        'unwrapped',false,'heterogeneous',true,'interactionTypes',{interactionTypes},'interactionMatrix',interactionMatrix,'backtracking',false); 
                
        
        if (~intraEdges)
             % FullBNLearn() will write current TSBN to GML and TGF if verbose is TRUE

             [BNet, ~] = FullBNLearn(data, cols_names, pheno, BFTHRESH, filename, ...
...
%                DBN.WriteToTGF(dbn_filename);
            ...
            [DBN, ~] = FullBNLearn(data, cols_names, pheno, BFTHRESH, dbn_filename, ...
